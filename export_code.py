...
def export_python_code_to_txt(project_root_dir, output_file_name="project_code.txt"):
    ...
    # Расширения файлов, которые будут собраны
    included_extensions = ('.py', '.c', '.cpp', '.h', '.hpp', '.html', '.htm', '.css', '.js',
                           '.json', '.xml', '.yaml', '.yml', '.txt', '.md', '.rst', '.ini',
                           '.cfg', '.conf', '.toml', '.sql', '.sh', '.bash', '.bat', '.ps1',
                           '.java', '.kt', '.kts', '.scala', '.go', '.rs', '.php', '.rb', '.pl',
                           '.lua', '.r', '.m', '.swift', '.vb', '.cs', '.fs', '.jl', '.exs', '.ex')
            ...
            for file in files:
                if file.endswith(included_extensions):
                    ...
                    try:
                        ...
                        # Пробуем другие кодировки для текстовых файлов
                        try:
                            with open(full_path, 'r', encoding='cp1251') as infile:
                                code_content = infile.read()
                                outfile.write(code_content)
                            outfile.write("\n" + "=" * 80 + "\n\n")
                            print(f"  Добавлен файл (cp1251): {relative_path}")
                        except Exception:
